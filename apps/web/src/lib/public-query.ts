...
export async function listPaginated(
  ...
  const countResult = kind
    ? await db
        .prepare(`SELECT COUNT(*) as count FROM artifacts WHERE is_active = 1 AND kind = ?1`)
        .bind(kind)
        .first<{ count: number }>()
    : await db
  ...
  const artifacts = results?.results ?? [];
